...
fn en_dict() -> HashMap<&'static str, &'static str> {
    HashMap::from([
        ...
        ("record.start", "Record"),
        ("sidebar.hide", "Hide meetings"),
        ("sidebar.show", "Show meetings"),
        ("record.pause", "Pause"),
        ...
        ("settings.shortcut", "Record shortcut"),
        ("shortcut.taken", "Another application already owns that one. The previous shortcut is still in place."),
        ("shortcut.none", "Neither combination could be registered, so there is no global shortcut right now. Pick another one."),
        ("shortcut.in_use_note", "It only works while Vesper is focused until you pick one nothing else owns."),
        ("processing.saving", "Saving the recording…"),
        ("processing.transcribing", "Transcribing…"),
        ("processing.final_pass", "Reading the whole recording again…"),
        ("processing.summarizing", "Writing the summary…"),
        ("processing.summary_failed", "The recording is saved — the summary failed."),
        ("processing.final_pass_failed", "This is the live transcript — the second pass over the recording failed."),
        ...
        ("tab.summary", "Summary"),
        ("tab.chat", "Chat"),
        ("summary.template", "Kind of meeting"),
        ("template.general", "General meeting"),
        ("template.standup", "Standup"),
        ("template.one_on_one", "One-on-one"),
        ("template.client_call", "Client call"),
        ("actions.owner", "Owner"),
        ("actions.due", "When"),
        ("actions.add", "Add a task"),
        ("actions.remove", "Remove task"),
        ("tab.notes", "Notes"),
        ("notes.title", "Your notes"),
        ("notes.subtitle", "Yours, not the model's"),
        ...
        ("context.hint", "Names, decisions, corrections. Kept on this meeting and read back when it is summarized."),
        ("chat.empty", "Ask about this meeting"),
        ("chat.empty_body", "Questions are answered from this meeting's transcript and summary, on your machine."),
        ("chat.placeholder", "Ask about this meeting…"),
        ("chat.send", "Send"),
        ("chat.thinking", "Reading the meeting to answer"),
        ("chat.answer", "Answer"),
        ("chat.jump_latest", "Jump to latest"),
        ("update.available", "Version {version} is available."),
        ("update.install", "Install and restart"),
        ("update.restart", "Restart to update"),
        ("update.busy", "Finish the recording before updating"),
        ("update.later", "Later"),
        ...
        ("cuda.installed", "NVIDIA acceleration is installed. Summaries now run on the {gpu}."),
        ("summary.working", "Reading the meeting"),
        ("summary.working_body", "Writing the overview, the key points and the action items. This runs on your machine, so it takes a few seconds — longer without a GPU."),
        ...
        ("cuda.title", "NVIDIA acceleration (CUDA)"),
        ("cuda.body", "Found a {gpu}. Vesper already uses this card through Vulkan; the CUDA pack is optional and speeds up summarising. It is not in the installer because it is large."),
        ("settings.save", "Save settings"),
        ("settings.stt_provider", "STT provider"),
        ("settings.llm_provider", "LLM provider"),
        ("settings.endpoint_url", "Server address"),
        ("settings.endpoint_model", "Model name on that server"),
        ("settings.endpoint_hint", "Ollama, LM Studio or any OpenAI-compatible server. It has to be on this machine or this network — a public address would send the transcript off it."),
        ("settings.reasoning", "Reasoning / CoT (OpenRouter)"),
        ("settings.auto_summarize", "Auto-summarize after recording"),
        ("settings.offline_mode", "Offline mode — nothing leaves this machine"),
        ...
        ("egress.text", "Text leaves this machine"),
        ("egress.both", "Audio and text leave this machine"),
        ("data.title", "Your meetings"),
        ("data.body", "Everything is on this computer. Export it before you delete it — there is no copy anywhere else."),
        ("data.export_all", "Export everything"),
        ("data.wipe", "Delete everything"),
        ("data.exported", "{count} meetings written."),
        ("data.wiped", "{count} meetings deleted."),
        ("data.confirm_title", "Delete every meeting?"),
        ("data.confirm_body", "Every transcript, summary, note, task and audio recording is deleted from this computer. Your settings and API key stay. This cannot be undone."),
        ("settings.offline_cloud_picked", "This section is still pointed at OpenRouter, which offline mode refuses."),
        ("settings.offline_use_local", "Use a local model"),
        ("settings.offline_no_download", "Downloads are off while offline mode is on."),
        ("settings.transcription_only", "Off, Vesper records and transcribes and never loads a language model. Summarizing and chat still work when you ask for them."),
        ("settings.final_stt_pass", "Read the recording again when it stops"),
        ("settings.final_stt_pass_on", "The live transcript is written while you talk, in pieces. When the recording stops, Vesper reads the whole thing once more and replaces it — the same words, heard with the rest of the meeting around them. It takes a while on a long recording, and it runs on this machine."),
        ("settings.final_stt_pass_cloud", "Only with local transcription. A cloud transcriber has already been paid for every sentence of this meeting, and reading it again would send the audio — and the bill — a second time."),
        ("settings.hot_words", "Words to get right"),
        ("settings.hot_words_hint", "One per line: names, products, jargon. They are given to the model as context before it listens, which makes it lean towards them — it is a nudge, not a rule, and a model that cannot hear the difference will still get it wrong."),
        ("settings.hot_words_placeholder", "Vesper\nOpenRouter\nwhisper.cpp"),
        ...
        // The floating card's dock. Named by where the card ends up, not by an
        // axis: "top right" is a place, "vertical alignment" is a form field.
        ("settings.overlay_position", "Where the recording card sits"),
        ("overlay.right_top", "Right edge, top"),
        ("overlay.right_center", "Right edge, middle"),
        ("overlay.right_bottom", "Right edge, bottom"),
        ("overlay.top", "Top edge, like a notch"),
        ("settings.close_to_tray", "Closing the window keeps Vesper in the notification area"),
        ("settings.close_to_tray_on", "The window hides instead of quitting, and a recording carries on. Quit from the tray icon."),
        ("backend.auto", "Automatic"),
        ("backend.cuda", "NVIDIA (CUDA)"),
        ("backend.vulkan", "Any GPU (Vulkan)"),
        ("backend.cpu", "CPU only"),
        ("backend.unavailable", "not in this build — using the best available"),
        ("cap.cuda", "GPU"),
        ("cap.cpu", "CPU"),
...
fn pt_dict() -> HashMap<&'static str, &'static str> {
    HashMap::from([
        ...
        ("record.start", "Gravar"),
        ("sidebar.hide", "Ocultar reuniões"),
        ("sidebar.show", "Mostrar reuniões"),
        ("record.pause", "Pausar"),
        ("record.resume", "Retomar"),
        ("record.stop", "Parar"),
        ("record.recording_badge", "Gravando"),
        ("record.paused_badge", "Pausado"),
        ("shortcut.unavailable", "Ctrl+Shift+R está em uso por outro app, então só funciona com a Vesper em foco."),
        ("settings.shortcut", "Atalho de gravação"),
        ("shortcut.taken", "Outro aplicativo já usa esse. O atalho anterior continua valendo."),
        ("shortcut.none", "Nenhuma das duas combinações pôde ser registrada, então não há atalho global agora. Escolha outra."),
        ("shortcut.in_use_note", "Ele só funciona com a Vesper em foco até você escolher um que ninguém mais use."),
        ("processing.saving", "Salvando a gravação…"),
        ("processing.transcribing", "Transcrevendo…"),
        ("processing.final_pass", "Relendo a gravação inteira…"),
        ("processing.summarizing", "Escrevendo o resumo…"),
        ("processing.summary_failed", "A gravação está salva — o resumo falhou."),
        ("processing.final_pass_failed", "Esta é a transcrição ao vivo — a segunda passagem pela gravação falhou."),
        ...
        ("tab.summary", "Resumo"),
        ("tab.chat", "Conversa"),
        ("summary.template", "Tipo de reunião"),
        ("template.general", "Reunião geral"),
        ("template.standup", "Daily"),
        ("template.one_on_one", "Um a um"),
        ("template.client_call", "Chamada com cliente"),
        ("actions.owner", "Responsável"),
        ("actions.due", "Quando"),
        ("actions.add", "Adicionar tarefa"),
        ("actions.remove", "Remover tarefa"),
        ("tab.notes", "Notas"),
        ("notes.title", "Suas notas"),
        ("notes.subtitle", "Suas, não do modelo"),
        ...
        ("context.hint", "Nomes, decisões, correções. Ficam nesta reunião e são lidas de volta quando ela for resumida."),
        ("chat.empty", "Pergunte sobre esta reunião"),
        ("chat.empty_body", "As respostas saem da transcrição e do resumo desta reunião, na sua máquina."),
        ("chat.placeholder", "Pergunte sobre esta reunião…"),
        ("chat.send", "Enviar"),
        ("chat.thinking", "Lendo a reunião para responder"),
        ("chat.answer", "Resposta"),
        ("chat.jump_latest", "Ir ao mais recente"),
        ...
        ("update.busy", "Termine a gravação antes de atualizar"),
        ("update.later", "Depois"),
        ...
        ("cuda.installed", "Aceleração NVIDIA instalada. Os resumos agora rodam na {gpu}."),
        ("summary.working", "Lendo a reunião"),
        ("summary.working_body", "Escrevendo a visão geral, os pontos principais e as ações. Isso roda na sua máquina, então leva alguns segundos — mais ainda sem GPU."),
        ...
        ("cuda.title", "Aceleração NVIDIA (CUDA)"),
        ("cuda.body", "Detectamos uma {gpu}. O Vesper já usa essa placa por Vulkan; o pacote CUDA é opcional e acelera o resumo. Fica fora do instalador porque é grande."),
        ("settings.save", "Salvar"),
        ("settings.stt_provider", "Provedor de STT"),
        ("settings.llm_provider", "Provedor de LLM"),
        ("settings.endpoint_url", "Endereço do servidor"),
        ("settings.endpoint_model", "Nome do modelo nesse servidor"),
        ("settings.endpoint_hint", "Ollama, LM Studio ou qualquer servidor compatível com OpenAI. Precisa estar nesta máquina ou nesta rede — endereço público mandaria a transcrição para fora dela."),
        ("settings.reasoning", "Raciocínio / CoT (OpenRouter)"),
        ("settings.auto_summarize", "Resumir automaticamente após gravar"),
        ("settings.offline_mode", "Modo offline — nada sai desta máquina"),
        ...
        ("data.title", "Suas reuniões"),
        ("data.body", "Está tudo neste computador. Exporte antes de apagar — não existe cópia em lugar nenhum."),
        ("data.export_all", "Exportar tudo"),
        ("data.wipe", "Apagar tudo"),
        ("data.exported", "{count} reuniões gravadas."),
        ("data.wiped", "{count} reuniões apagadas."),
        ("data.confirm_title", "Apagar todas as reuniões?"),
        ("data.confirm_body", "Toda transcrição, resumo, nota, tarefa e gravação de áudio é apagada deste computador. Suas configurações e a chave de API permanecem. Isso não tem volta."),
        ("settings.offline_cloud_picked", "Esta seção ainda aponta para a OpenRouter, que o modo offline recusa."),
        ("settings.offline_use_local", "Usar um modelo local"),
        ("settings.offline_no_download", "Download está desligado enquanto o modo offline estiver ligado."),
        ("settings.transcription_only", "Desligado, o Vesper grava e transcreve sem nunca carregar um modelo de linguagem. Resumir e conversar continuam funcionando quando você pedir."),
        ("settings.final_stt_pass", "Reler a gravação quando ela parar"),
        ("settings.final_stt_pass_on", "A transcrição ao vivo é escrita enquanto você fala, aos pedaços. Quando a gravação para, a Vesper lê tudo de novo e substitui — as mesmas palavras, ouvidas com o resto da reunião em volta. Demora um pouco numa gravação longa, e roda nesta máquina."),
        ("settings.final_stt_pass_cloud", "Só com transcrição local. Um transcritor de nuvem já foi pago por cada frase desta reunião, e reler mandaria o áudio — e a conta — uma segunda vez."),
        ("settings.hot_words", "Palavras para acertar"),
        ("settings.hot_words_hint", "Uma por linha: nomes, produtos, jargão. Elas vão para o modelo como contexto antes de ele ouvir, o que faz ele pender para elas — é um empurrão, não uma regra, e um modelo que não consegue ouvir a diferença ainda vai errar."),
        ("settings.hot_words_placeholder", "Vesper\nOpenRouter\nwhisper.cpp"),
        ...
        ("settings.overlay_position", "Onde fica o cartão de gravação"),
        ("overlay.right_top", "Borda direita, em cima"),
        ("overlay.right_center", "Borda direita, no meio"),
        ("overlay.right_bottom", "Borda direita, embaixo"),
        ("overlay.top", "Borda de cima, como um notch"),
        ("settings.close_to_tray", "Fechar a janela mantém a Vesper na bandeja"),
        ("settings.close_to_tray_on", "A janela some em vez de sair, e uma gravação continua. Para sair de verdade, use o ícone na bandeja."),
        ("backend.auto", "Automático"),
        ("backend.cuda", "NVIDIA (CUDA)"),
        ("backend.vulkan", "Qualquer GPU (Vulkan)"),
        ("backend.cpu", "Somente CPU"),
        ("backend.unavailable", "não está nesta versão — usando a melhor disponível"),
        ("cap.cuda", "GPU"),
        ("cap.cpu", "CPU"),
